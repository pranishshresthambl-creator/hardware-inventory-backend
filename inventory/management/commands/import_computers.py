...
from api.import_views import BulkImportComputerView
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            request = factory.post("/api/computers/import-excel/", {"file": uploaded_file}, format="multipart")
            view = BulkImportComputerView.as_view()
            response = view(request)

            if response.status_code == 200:
                data = response.data
                self.stdout.write(self.style.SUCCESS(
                    f"Successfully processed {data.get('total_processed', 0)} computers: "
